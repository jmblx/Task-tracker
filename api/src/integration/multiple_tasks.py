...
async def get_project_description(project_id):
    ...
    configuration.access_token = (
        "2/1207064323890384/1207066247133573:bd4780cfc1b0c461d853e443d0fbe028"
    )
    opts = {"opt_fields": "notes"}
...
async def get_task_details(project_id, organization_id):
    ...
    opts = {"limit": 50, "project": project_id, "opt_fields": ",".join(opt_fields)}
    ...
    for task in tasks_data:
        ...
        task_details = {
            ...
            "custom_fields": {
                cf["name"]: cf["display_value"] for cf in task.get("custom_fields", [])
            },
            "section": next(
                (
                    m["section"]["name"]
    ...
    async with async_session_maker() as session:
        ...
        for gid, task_details in tasks_summary.get("tasks").items():
            ...
            done_at_naive = (
                make_naive(task_details.get("completed_at"))
            ...
            if author_email:
                author = (
                    await session.execute(
                        select(User).where(User.email == author_email)
                    )
                ).scalar()

                diff = (
                    "easy"
                    if task_details["custom_fields"].get("Приоритет", "") == "Низкий"
                    else "hard"
                )
