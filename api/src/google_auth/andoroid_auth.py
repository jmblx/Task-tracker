...
from gql.graphql_utils import get_func_data, process_data_and_insert
...
def google_register(func: Callable) -> Callable:
    async def wrapper(self, info: 'Info', data: 'GoogleRegDTO') -> 'UserType':
        ...
        data_dict["last_name"] = data_dict.pop("familyName") if data_dict.get("familyName") is not None else None
        print(data_dict)

        obj, _, selected_fields = await process_data_and_insert(info, User, data_dict, function_name=function_name)
