...
def create_jwt(
    ...
    expire_minutes = expire_minutes or auth_settings.access_token_expire_minutes
    jwt_payload = {TOKEN_TYPE_FIELD: token_type}
    jwt_payload.update(token_data)
...
async def create_refresh_token(user: Any, fingerprint: str, auth_settings: "AuthJWT") -> dict:
    jti = str(uuid4())
    jwt_payload = {
        "sub": str(user.id),
        "jti": jti,
...
async def authenticate(redis, info: Info, user: User, auth_settings: "AuthJWT") -> tuple[Response, JSON]:
    access_token = create_access_token(user, auth_settings)
    fingerprint = info.context.get("fingerprint", None)
    if not fingerprint:
        raise HTTPException(HTTP_400_BAD_REQUEST, detail="Fingerprint is required")

    refresh_token_data = await create_refresh_token(user, fingerprint, auth_settings)
